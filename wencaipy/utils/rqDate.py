...
rqTZInfo_CN = 'Asia/Shanghai'

def rq_util_time_now():
    """
    explanation:
       获取当前日期时间
    return:
...
def rq_util_today_str():
    ...
    return:
        str
    """
    dt = rq_util_date_today()
    return rq_util_datetime_to_strdate(dt)


def rq_util_date_str2int(date):
    """
    explanation:
        转换日期字符串为整数
    params:
        * date->
            ...
            参数支持: []
    demonstrate:
        print(rq_util_date_str2int("2011-09-11"))
    return:
        int
    output:
        >>20110911
...
def rq_util_date_int2str(int_date):
    """
    explanation:
        转换日期整数为字符串
    params:
...
def rq_util_datetime_to_strdate(dt):
    """
    explanation:
        转换字符串格式的日期为datetime
    params:
...
def rq_util_datetime_to_strdatetime(dt):
    """
    explanation:
        转换日期时间为字符串格式
    params:
...
def rq_util_tdxtimestamp(time_stamp):
    """
    explanation:
...
def rq_util_pands_timestamp_to_date(pandsTimestamp):
    """
    explanation:
...
def rq_util_pands_timestamp_to_datetime(pandsTimestamp):
    """
    explanation:
...
def rq_util_stamp2datetime(timestamp):
    """
    explanation:
...
def rq_util_realtime(strtime, client):
    """
    explanation:
        查询数据库中的数据
    params:
    ...
    return:
        dict
    """
    time_stamp = rq_util_date_stamp(strtime)
    coll = client.rrdata.trade_date
...
def rq_util_id2date(idx, client):
    """
    explanation:
         从数据库中查询通达信时间
    params:
...
def rq_util_is_trade(date, code, client):
    """
    explanation:
        从数据库中查询判断是否是交易日
    params:
...
def rq_util_get_date_index(date, trade_list):
    """
    explanation:
...
def rq_util_get_index_date(id, trade_list):
    """
    explanation:
        根据id索引值
    params:
...
def rq_util_select_hours(time=None, gt=None, lt=None, gte=None, lte=None):
    """
    explanation:
        rrdata的时间选择函数,约定时间的范围,比如早上9点到11点
    params:
        * time->
            含义: 时间
            类型: str
...
def rq_util_select_min(time=None, gt=None, lt=None, gte=None, lte=None):
    """
    explanation:
        择分钟
    params:
...
def rq_util_time_delay(time_=0):
    """
    explanation:
...
def rq_util_calc_time(func, *args, **kwargs):
    """
    explanation:
        耗时长度的装饰器
    params:
