...
from wencaipy.utils.rqParameter import FREQUENCE, MARKET_TYPE
from wencaipy.utils.tradeDate import trade_date_cn

from wencaipy.utils.tradeDate import trade_date_sse

trade_date_sse = trade_date_cn
...
def rq_util_if_traded_now():
    ...
    """
    _now = datetime.datetime.now()
    _now_time =  datetime.datetime.strptime(str(_now)[0:19], "%Y-%m-%d %H:%M:%S")
    print(_now_time)
    
    if (datetime.date.today() in trade_date_sse) and (datetime.datetime.now() > datetime.time(9, 25, 0)):
        return True
    else:
        return False
...
if __name__ == "__main__":
    ...
    print(rq_util_if_trade(datetime.date.today()))
    #print(rq_util_get_trade_datetime(dt=datetime.datetime.now()))
    #print(rq_util_get_real_date("2022-06-30"))
    print(rq_util_if_traded_now())
